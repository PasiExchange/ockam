...
use crate::channel::encryptor::EncryptorWorker;
...
impl<V: IdentityVault, S: AuthenticatedStorage> DecryptorWorker<V, XXResponder<V>, S> {
    pub(crate) async fn create_responder(
        ...
        let worker = DecryptorWorker {
            ...
            addresses: addresses.clone(),
            remote_route,
            remote_backwards_compatibility_address: Some(first_responder_address),
    ...
    DecryptorWorker<V, K, S>
{
    ...
    async fn handle_key_exchange(
        ...
        let state = self.state_key_exchange.take().unwrap(); // FIXME
        ...
        let state = ExchangeIdentity {
            ...
            decryption_helper: DecryptionHelper::new(
    ...
    DecryptorWorker<V, K, S>
{
    ...
    async fn handle_exchange_identity(
        ...
        if let Some(msg) = msg {
            ...
            let body = state.decryption_helper.decrypt(&body).await?;
        ...
        if !state.identity_sent {
            ...
            let msg = TransportMessage::v1(
                self.remote_backwards_compatibility_address.clone().unwrap(), // FIXME
                route![self.addresses.decryptor_backwards_compatibility.clone()],
                auth_msg.encode()?,
            );
            let data = msg.encode()?;

            let encrypted = state.encryption_helper.encrypt(&data).await?;
        ...
        if let Some(their_identity_id) = state.received_identity_id.clone() {
            let old_state = self.state_exchange_identity.take().unwrap();
            self.state_initialized = Some(Initialized {
                decryption_helper: old_state.decryption_helper,
    ...
    DecryptorWorker<V, K, S>
{
    async fn handle_decrypt(
        ...
        let decrypted_payload = state.decryption_helper.decrypt(&payload).await?;

        let mut transport_message = TransportMessage::decode(&decrypted_payload)?;

        // Ensure message goes to backwards compatibility address
        if transport_message.onward_route.next()?
            != &self.addresses.decryptor_backwards_compatibility
        {
            return Err(IdentityError::UnknownChannelMsgDestination.into());
        }

        // Skip backwards compatibility address
        let _ = transport_message.onward_route.step()?;

        // Replace decryptor hop in the return_route with the encryptor address
        transport_message
            .return_route
            .modify()
            .pop_front()
